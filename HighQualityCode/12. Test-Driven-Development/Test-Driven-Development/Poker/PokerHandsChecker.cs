namespace Poker
{
    ...
    public class PokerHandsChecker : IPokerHandsChecker
    {
        ...
        public int CompareHands(IHand firstHand, IHand secondHand)
        {
            ...
            {
                throw new ArgumentException("One of the hands is invalid! You cannot compare invalid hands.");
            }

            Categories firstHandCategories = FindCategories(firstHand);
            Categories secondHandCategories = FindCategories(secondHand);
            int result = firstHandCategories.CompareTo(secondHandCategories);

            return result;
        }

        public Categories FindCategories(IHand hand)
        {
            Categories combination = Categories.HighCard;
            if (IsOnePair(hand))
            {
                combination = Categories.OnePair;
            }
            else if (IsTwoPair(hand))
            {
                combination = Categories.TwoPair;
            }
            else if (IsThreeOfAKind(hand))
            {
                combination = Categories.ThreeOfAKind;
            }
            else if (IsStraight(hand))
            {
                combination = Categories.Straight;
            }
            else if (IsFlush(hand))
            {
                combination = Categories.Flush;
            }
            else if (IsFullHouse(hand))
            {
                combination = Categories.FullHouse;
            }
            else if (IsFourOfAKind(hand))
            {
                combination = Categories.FourOfAKind;
            }
            else if (IsStraightFlush(hand))
            {
                combination = Categories.StraightFlush;
            }
